...
import { deleteActivityEmbeddings, insertActivityEmbedding } from '../dal/activities';
import { ActivityEmbedding } from '../../types/types';
import { generateEmbeddings } from './open-ai';
import { getAllCategories, insertCategoryEmbedding } from '../dal/categories';
...
export const embedActivities = async (activity: Pick<Activities, 'id' | 'description'>) => {
  await deleteActivityEmbeddings(activity.id);

  if (activity.description.length < 10) {
    return [];
  }
  ...
  for (let i = 0; i < chunkedReviews.length; i++) {
    ...
    //   -0.02357791,
    // ];
    const embedding = await generateEmbeddings(chunkedReviews[i].content);
...
export const embedAllCategories = async () => {
  const categories = await getAllCategories();
  for (let category of categories) {
    const embedding = await generateEmbeddings(category.name + ', ' + category.description);
    await insertCategoryEmbedding(category, embedding);
  }
};
